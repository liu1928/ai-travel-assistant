...
## 2026-07-21（八）：Trip Day Mode（`specs/trip-day-mode.md`，組裝件，最後一份）

> 任務來源：「都做」批次最後一份，硬依賴 schedule-anchoring（已完成）、軟依賴 flight-day-status/
> map-view（皆已完成，直接複用）。5 檔改動（`app/trips/[id]/page.tsx` 為主）。
> ⚠️ 兩處判斷需要說明：
> 1. **導航按鈕升級是全域的，不是只在今日卡片**：spec 把「有座標的 stop 加導航」寫在「今日卡片強化」
>    段落，但既有 `navUrl()` 是所有天共用的同一個函式。判斷：把 `navUrl` 升級成「有座標走精確 dir
>    deep link、沒座標退回既有文字搜尋」對所有天都是純粹的品質提升（不會讓任何天變差），比只在今日
>    的 item 上特判更簡單一致，符合 spec §2「增強而非改版」的精神。
> 2. **今日天氣沿用既有的 per-day 索引對齊邏輯，不另開日期比對**：spec 1.2 寫「從 weather[] 以日期
>    比對取當日」，但天氣/匯率那輪（2026-07-20）已經是 index 對齊（`weather[day.day-1]`）。今天對應
>    的 day 就是 `tripDay`，`weather[tripDay-1]` 跟「日期比對取今天」在資料語意上是同一件事（同一天
>    索引位置只可能對應同一個日期）——重新寫一套日期比對只是同一份資料的另一種查法，沒有實益，故直接
>    複用既有 `dayWeather` 邏輯，不新增第二套查詢路徑。

1. **`lib/trip-day.ts`（新）**：`currentTripDay(startDate, totalDays, today)` 純函式（UTC 午夜運算同 `dateForDay`/`daysDiff` 慣例）；`findNextStopIndex(schedule, nowHHMM)`（複用 `lib/trip-edit.ts` 的 `timeToMin`）；`todayLocalDateStr()`（client 本地日）。
2. **`components/bookings.tsx`**：`FlightStatusRow` 加 `export`（供本頁複用 flight-day-status 的查即時動態能力，spec 明講「不重複實作」）。
3. **`app/trips/[id]/page.tsx`**：
   - `navUrl()` 升級：有 `lat/lng` 走 `maps/dir/?api=1&destination=...`（有 `placeId` 加 `destination_place_id`），無座標退回既有文字搜尋。
   - 頁首徽章「🧭 旅途中 · 第 N 天／共 M 天」（`tripDay!==null` 才顯示）。
   - 進頁 `useEffect` 捲動並高亮到 `id="day-{tripDay}"` 卡片（`ring-2 ring-teal-500`），其他天照常可看。
   - 今日卡片：「今天」徽章、`findNextStopIndex` 算出的「下一站」標籤、地圖 toggle 預設展開（`useEffect` 只在 `tripDay` 首次解出時強制加開一次，之後使用者手動收合不會被彈開）。
   - 今日航班：`flights[].date===today` 者，渲染成置頂今日卡片上方的「✈️ 今日航班」卡片，內含 `<FlightStatusRow>`。
4. **`lib/__tests__/trip-day.test.ts`（新）**：15 條（`currentTripDay` 首日/末日/期間內外/跨月/缺 startDate/格式不合、`findNextStopIndex` 邊界、`todayLocalDateStr` 格式）。

範圍外（spec 已註記）：不做推播/鬧鐘提醒；不取 geolocation（導航交給 Google Maps）；不做行程自動打卡。

驗證：`pnpm typecheck && pnpm test && pnpm lint && pnpm build` 全綠 → diff → GLM review → REVIEW/REPORT。**人工實測缺口**：需要一筆 `startDate` 落在今天附近的真實行程才能在瀏覽器裡看到旅途模式的視覺效果（徽章/捲動/下一站/今日航班），本輪僅能靠 build+typecheck+單測驗證邏輯正確、無執行期錯誤，視覺驗收留給 peanut 部署後用真實資料測（可把某行程 `startDate` 手動改成今天附近驗證，spec 本身也是這樣建議的）。
